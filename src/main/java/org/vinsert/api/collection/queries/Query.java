...
public abstract class Query<T> implements Iterable<T> {
    ...
    private final List<Comparator<T>> orderings = new ArrayList<Comparator<T>>();
    ...
    Query retrieveFromStorage(String name) {
        ...
        throw new IllegalStateException("Query '" + name + "' not found in storage, call storeAs(String name) first.");
    }
